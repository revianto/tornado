...
class Tank_auth
{
	...
	//LDAP FUNCTION AUTH
	function get_user_ldap($username, $password)
	{
		if (!is_null($user = $this->ci->users->get_user_by_login($username))) {
			// login ok
			// Does password match hash in database?
			$hasher = new PasswordHash(
				$this->ci->config->item('phpass_hash_strength', 'tank_auth'),
	...
	function change_password_ldap($username, $password)
	{
		if ((strlen($username) > 0) and (strlen($password) > 0)) {
			if (!is_null($user = $this->ci->users->get_user_by_login($username, TRUE))) {
				// Hash password using phpass
				$hasher = new PasswordHash(
					$this->ci->config->item('phpass_hash_strength', 'tank_auth'),
					$this->ci->config->item('phpass_hash_portable', 'tank_auth')
				...
				if ($this->ci->users->reset_password_ldap(
					$username,
					$hashed_password)) {
					// success
					return array(
	...
	function get_username_ldap($username)
	{
		$data = [
			'username' => $username
		];

		return $this->ci->users->check_ldap($data);
	}
	 ...
	 * @return    bool
	 */
	function activate_user_manual($user_id, $activate_by_email = TRUE)
	{
		if (strlen($user_id) > 0) {
			return $this->ci->users->activate_user_manual($user_id, $activate_by_email);
		}
		return FALSE;
	}
	function non_activate_user_manual($user_id, $activate_by_email = TRUE)
	{
		if (strlen($user_id) > 1) {
			return $this->ci->users->activate_user_manual($user_id, $activate_by_email);
		}
	...
	{
		return $this->ci->users->get_list_role();
	}
